...
class CatBrain {
    ...
    constructor(options) {
        ...
        // Hidden layer init
        this.weightsInputToHidden = Array.from({ length: this.hiddenAmount }, () => Array.from({ length: this.inputAmount }, () => Math.random() * 2 - 1));
        this.biasHidden = Array(this.hiddenAmount).fill(0);
        this.hiddenLayer = new Array(this.hiddenAmount).fill(0);
        // Output init
        this.weightsHiddenToOutput = Array.from({ length: this.outputAmount }, () => Array.from({ length: this.hiddenAmount }, () => Math.random() * 2 - 1));
